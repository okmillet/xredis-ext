...
bool RedisPool::ConnectRedisDB( uint32_t cahcetype,  uint32_t nodeIndex,
    const std::string& host, uint32_t port, const std::string& passwd,
	uint32_t dbIndex, uint32_t poolsize,  uint32_t timeout, uint32_t role){
    if( (0==host.length())
        || (cahcetype>MAX_REDIS_CACHE_TYPE)
        || (nodeIndex>MAX_REDIS_DB_HASHBASE)
        || (cahcetype>mTypeSize - 1)
        || (role>SLAVE)
        || (poolsize>MAX_REDIS_CONN_POOLSIZE)) {
            return false;
    }
    return mRedisCacheList[cahcetype].ConnectRedisDB(cahcetype, nodeIndex, host, port,
        passwd, dbIndex, poolsize, timeout, role);
}
...
RedisConn *RedisPool::GetConnection(uint32_t cahcetype, uint32_t nodeIndex, uint32_t ioType){
    RedisConn *pRedisConn = NULL;

    if ( (cahcetype>mTypeSize)
        ||(nodeIndex>mRedisCacheList[cahcetype].GetHashBase())
        || (ioType>SLAVE)
        ){
        return NULL;
    }
    ...
    pRedisConn = pRedisCache->GetConn(nodeIndex, ioType);
...
bool RedisConn::select()
{
    bool bRet = false;
    if (mDBindex == 0) {
        bRet = true;
    }else{
        redisReply *reply = static_cast<redisReply *>(redisCommand(mCtx, "SELECT %d", mDBindex));
        if ((NULL == reply) || (strcasecmp(reply->str, "OK") != 0)) {
            bRet = false;
        } else {
            bRet = true;
...
void RedisConn::Init(uint32_t cahcetype, uint32_t nodeIndex, const std::string& host, uint32_t port,
    const std::string& pass, uint32_t dbIndex, uint32_t poolsize, uint32_t timeout, uint32_t role, uint32_t slaveidx)
{
    mType = cahcetype;
...
void RedisDBSlice::Init( uint32_t cahcetype,  uint32_t nodeIndex)
{
    mType = cahcetype;
    mNodeIndex = nodeIndex;
}

bool RedisDBSlice::ConnectRedisNodes(uint32_t cahcetype, uint32_t nodeIndex, const std::string& host, uint32_t port,
    const std::string& passwd, uint32_t dbIndex, uint32_t poolsize, uint32_t timeout, int32_t role)
{
    bool bRet = false;
    if ((host.empty())
        || (cahcetype > MAX_REDIS_CACHE_TYPE)
    ...
    try {
        if (MASTER == role) {
            ...
            for (uint32_t i = 0; i < poolsize; ++i) {
                ...
                pRedisconn->Init(cahcetype, nodeIndex, host, port, passwd, dbIndex, poolsize, timeout, role, 0);
            ...
            for (uint32_t i = 0; i < poolsize; ++i) {
                ...
                pRedisconn->Init(cahcetype, nodeIndex, host, port, passwd, 0, poolsize, timeout, role, slave_idx);
                if (pRedisconn->RedisConnect()) {
...
bool RedisCache::ConnectRedisDB(uint32_t cahcetype, uint32_t nodeIndex, const std::string& host,
    uint32_t port, const std::string& passwd, uint32_t dbIndex, uint32_t poolsize, uint32_t timeout, uint32_t role)
{
    mDBList[nodeIndex].Init(cahcetype, nodeIndex);
    return mDBList[nodeIndex].ConnectRedisNodes(cahcetype, nodeIndex, host, port,
        passwd, dbIndex, poolsize, timeout, role);
}
...
uint32_t RedisCache::GetDBStatus(uint32_t nodeIndex)
{
    RedisDBSlice *pdbSclice = &mDBList[nodeIndex];
    if (NULL == pdbSclice) {
...
{
    return mDBList[redisconn->getNodeIndex()].FreeConn(redisconn);
}

RedisConn * RedisCache::GetConn(uint32_t nodeIndex, uint32_t ioRole)
{
    return mDBList[nodeIndex].GetConn(ioRole);
}
