...
public:
    ...
    void Init( uint32_t  cahcetype, uint32_t  nodeIndex);
    // 连到到一个REDIS服务节点
    bool ConnectRedisNodes(uint32_t cahcetype, uint32_t nodeIndex,
                           const std::string& host,  uint32_t port, const std::string& passwd,
						   uint32_t dbIndex, uint32_t poolsize,  uint32_t timeout, int32_t role);
...
private:
    ...
    bool           mHaveSlave;
    uint32_t   mType;          // redis cache pool type
    uint32_t   mNodeIndex;       // redis slice index
...
public:
    ...
    bool ConnectRedisDB( uint32_t cahcetype,  uint32_t nodeIndex,
        const std::string& host, uint32_t port, const std::string& passwd,
		uint32_t dbIndex, uint32_t poolsize, uint32_t timeout, uint32_t role);

    RedisConn *GetConn( uint32_t nodeIndex, uint32_t ioRole);
    void FreeConn(RedisConn *redisconn);
    void ClosePool();
    void KeepAlive();
    uint32_t GetDBStatus(uint32_t nodeIndex);
...
public:
    ...
    bool ConnectRedisDB(uint32_t cachetype,  uint32_t nodeIndex,
        const std::string& host, uint32_t port, const std::string& passwd,
		uint32_t dbIndex, uint32_t poolsize,  uint32_t timeout, uint32_t role);
    static bool CheckReply(const redisReply* reply);
    static void FreeReply(const redisReply* reply);
    
    RedisConn *GetConnection(uint32_t cachetype, uint32_t nodeIndex, uint32_t ioType = MASTER);
