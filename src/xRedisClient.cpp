...
bool xRedisClient::ConnectRedisCache(const RedisNode *redisnodelist, uint32_t nodecount, uint32_t hashbase, uint32_t cachetype) {
    ...
    for (uint32_t n = 0; n<nodecount; n++) {
        ...
        bool bRet = mRedisPool->ConnectRedisDB(cachetype, pNode->nodeIndex, pNode->host, pNode->port,
            pNode->passwd, 0, pNode->poolsize, pNode->timeout, pNode->role);
        if (!bRet) {
            return false;
        }
    }
...
bool xRedisClient::ConnectRedisCache(const RedisNode *redisNode, uint32_t cachetype) {
    if (NULL==mRedisPool) {
        return false;
    }
    if (!mRedisPool->setHashBase(cachetype, 1)) {
        return false;
    }
	if (NULL==redisNode) {
		return false;
	}
	bool bRet = mRedisPool->ConnectRedisDB(cachetype, redisNode->nodeIndex, redisNode->host, redisNode->port,
		redisNode->passwd, redisNode->dbIndex, redisNode->poolsize, redisNode->timeout, redisNode->role);
	if (!bRet) {
		return false;
	}
    return true;
